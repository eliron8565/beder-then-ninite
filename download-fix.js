...
(function () {
  ...
  async function downloadWindows(chosen,button){
    ...
    try{
      ...
      while(true){const {done,value}=await reader.read();if(done)break;chunks.push(value);received+=value.length;if(button&&total>0)button.textContent=`Downloading AppForge… ${Math.min(99,Math.floor(received/total*100))}%`}
      if(received<10*1024*1024)throw new Error('Release file is incomplete');
      saveBlob(new Blob(chunks,{type:'application/vnd.microsoft.portable-executable'}),name);
      return;
    }catch(e){
      console.warn('GitHub Release fetch failed; using Pages fallback.',e);
      directDownload(`downloads/AppForge-Windows.exe?v=${Date.now()}`,name);
    }
  }
  ...
  function commandFor(chosen){if(platform==='windows'){const ids=chosen.map(a=>a.pkg.windows).filter(v=>v&&!String(v).startsWith('url:'));if(!ids.length)return '';return ids.map(id=>`winget install --id "${id}" -e --silent --accept-package-agreements --accept-source-agreements --disable-interactivity`).join(' ; ')}if(platform==='linux'){const ids=chosen.map(a=>a.pkg.linux).filter(Boolean);return ids.length?`flatpak install -y flathub ${ids.map(id=>`"${id}"`).join(' ')}`:''}const pkgs=chosen.map(a=>a.pkg.mac).filter(Boolean);if(!pkgs.length)return '';const formulas=pkgs.filter(p=>p.type==='formula').map(p=>p.id),casks=pkgs.filter(p=>p.type==='cask').map(p=>p.id),parts=[];if(formulas.length)parts.push(`brew install ${formulas.map(x=>`"${x}"`).join(' ')}`);if(casks.length)parts.push(`brew install --cask ${casks.map(x=>`"${x}"`).join(' ')}`);return parts.join(' ; ')}
  ...
  function wire(){const old=document.querySelector('#downloadScript');if(!old||old.dataset.nativeDownload==='1'){wireCommand();return}const button=old.cloneNode(true);button.dataset.nativeDownload='1';old.replaceWith(button);button.addEventListener('click',async()=>{const chosen=availableApps().filter(a=>selected.has(a.key));if(!chosen.length)return;const oldText=button.textContent;button.disabled=true;button.textContent=platform==='windows'?'Connecting to fast download…':'Preparing AppForge…';try{if(platform==='windows')await downloadWindows(chosen,button);else if(platform==='linux')await downloadLinux(chosen);else await downloadMac(chosen)}catch(e){alert(`AppForge: ${e.message}`)}finally{button.disabled=false;button.textContent=oldText}});wireCommand()}
  wire();
})();